    ...
    override fun bind(binding: ItemHandleContractBinding, item: Contract, position: Int) {
        binding.apply{
            ...
            if(item.isActive == HopDongEntity.ACTIVE){
                ...
                tvState.text = "Tình trạng hợp đồng: đã kết thúc"
            ...
            btnEnd.isVisible = (item.state == Contract.State.ACTIVE || item.state == Contract.State.NEAR_END) && item.isActive == HopDongEntity.ACTIVE
            btnRefresh.isVisible = (item.state == Contract.State.ENDED || item.state == Contract.State.NEAR_END || item.state == Contract.State.UNKNOWN) && item.isActive == HopDongEntity.ACTIVE

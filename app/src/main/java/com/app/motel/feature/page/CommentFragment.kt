...
class CommentFragment : AppBaseFragment<FragmentCommentBinding>() {
    ...
    private fun init() {
        adapter = CommentAdapter(
            ...
            settingController = viewModel.settingController,
    ...
    fun listenStateViewModel(){
        ...
        viewModel.settingController.state.backgroundColor.observe(viewLifecycleOwner){
            if(it == R.color.background2){
                views.lyAvatar.backgroundTintList = ContextCompat.getColorStateList(requireContext(), R.color.background1)
            }else{
                views.lyAvatar.backgroundTintList = ContextCompat.getColorStateList(requireContext(), R.color.background2)
            }

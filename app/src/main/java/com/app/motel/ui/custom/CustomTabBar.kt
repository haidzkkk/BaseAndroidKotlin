...
import com.history.vietnam.databinding.LayoutTabBarBinding
    ...
    private fun initTabTv(tabName: String, position: Int) = TextView(context).apply {
        ...
        setTextColor(
            ContextCompat.getColor(
                this.context,
                if (position == currentPosition) R.color.white else R.color.textColor_permanent_3
            )
        )
    ...
    fun setTabSelected(position: Int) {
        ...
        listTabTv.forEachIndexed { i, textView ->
            textView.setTextColor(
                ContextCompat.getColor(
                    context,
                    if (i == position) R.color.white else R.color.textColor_permanent_3
                )
            )
        }
    }
